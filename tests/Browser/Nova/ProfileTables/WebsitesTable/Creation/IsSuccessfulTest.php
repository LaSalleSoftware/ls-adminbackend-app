...
namespace Tests\Browser\Nova\ProfileTables\WebsitesTable\Creation;
...
class IsSuccessfulTest extends DuskTestCase
{
    ...
    public function setUp(): void
    {
        ...
        $this->newData = [
            'url'                     => 'https://www.allaboutbluesmusic.com/b-b-king/',
...
Sodales ut eu sem integer. Velit aliquet sagittis id consectetur purus ut faucibus pulvinar elementum. Neque vitae tempus quam pellentesque. Sed odio morbi quis commodo. Diam quis enim lobortis scelerisque fermentum. Morbi leo urna molestie at elementum eu facilisis sed. Sapien eget mi proin sed libero enim sed faucibus. Cursus metus aliquam eleifend mi in nulla posuere. In dictum non consectetur a. Amet dictum sit amet justo donec enim diam. Sagittis eu volutpat odio facilisis mauris sit amet massa vitae. Nisi porta lorem mollis aliquam ut porttitor leo a. Vitae sapien pellentesque habitant morbi tristique senectus et netus. Ac ut consequat semper viverra. Sagittis vitae et leo duis. Porta lorem mollis aliquam ut porttitor leo a diam. Aenean euismod elementum nisi quis eleifend quam adipiscing. Rutrum tellus pellentesque eu tincidunt.',
     ...
     * @group nova
     * @group novaswebsite
     * @group novaswebsitecreationissuccessful
     */
    public function testCreateNewRecordIsSuccessful()
    {
        echo "\n**Now testing Tests\Browser\Nova\ProfileTables\WebsitesTable\Creation\IsSuccessfulTest**";
        ...
        $this->browse(function (Browser $browser) use ($personTryingToLogin, $newData) {
            $browser->visit('/login')
                ...
                ->click('@create-button')
                ->pause(5000)
                ->assertSee('New Website')
                ->assertSelectHasOptions('@lookup_website_type', [1,2,3,4,5,6])
            ...
            $this->assertEquals($newData['url'],                    $website->url);
            ...
            //$this->assertEquals($newData['comments'],               $website->comments);  **the comments are encrypted in the db, so this direct comparison will fail**
        ...
        $this->assertDatabaseHas('websites', ['url'         => $newData['url']]);
        $this->assertDatabaseHas('websites', ['description' => $newData['description']]);
        //$this->assertDatabaseHas('websites', ['comments'    => $newData['comments']]);  **the comments are encrypted in the db, so this direct comparison will fail**
    }
}
